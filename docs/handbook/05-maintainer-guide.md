...
metaphor make entity Vendor --module corporate
...
metaphor schema schema validate corporate

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate corporate --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor corporate
...
> `corporate` is your module name (auto-detected from the current directory when omitted).
...
2. `metaphor schema schema validate corporate`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> corporate` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration corporate --database-url …`).
4. Regenerate code: `metaphor schema schema generate corporate --target all --force`.
