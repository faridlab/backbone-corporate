//! Application service for Territory entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `territory_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateTerritoryDto, UpdateTerritoryDto};
use crate::domain::entity::Territory;
use crate::infrastructure::persistence::TerritoryRepository;

/// Application service for Territory entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `territory_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct TerritoryServiceCustom {
///     inner: Arc<TerritoryService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type TerritoryService = GenericCrudService<
    Territory,
    CreateTerritoryDto,
    UpdateTerritoryDto,
    TerritoryRepository,
>;

// <<< CUSTOM
// END CUSTOM
