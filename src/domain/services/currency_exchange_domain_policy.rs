//! Domain policy for CurrencyExchange aggregate
//!
...
/// Domain policy for CurrencyExchange — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<CurrencyExchange>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type CurrencyExchangeDomainPolicy = PermitAllPolicy<CurrencyExchange>;
