...
metaphor migration generate Invoice corporate
metaphor migration run
...
(`corporate` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed corporate          # run Rust seeders
metaphor migration generate-seeds corporate  # emit SQL seed files
```
